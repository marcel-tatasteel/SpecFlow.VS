...
public abstract class ProjectGenerator : IProjectGenerator
{
    ...
    private void GenerateTestArtifacts(string projectFilePath)
    {
        ...
        var assetGenerator =
            new SpecFlowAssetGenerator(Math.Max(3, stepCount * _options.StepDefinitionPerStepPercent / 100));
    ...
    protected virtual void SetSpecFlowUnitTestProvider(ProjectChanger projectChanger, string packagesFolder)
    {
        if (_options.SpecFlowVersion >= new Version("3.0"))
        {
            ...
            ExecNuGetInstall("SpecFlow.Tools.MsBuild.Generation", packagesFolder, "-Version",
                _options.SpecFlowPackageVersion);
            InstallNuGetPackage(projectChanger, packagesFolder, "SpecFlow.Tools.MsBuild.Generation", sourcePlatform,
                _options.SpecFlowPackageVersion);

            ExecNuGetInstall("SpecFlow." + _options.UnitTestProvider, packagesFolder, "-Version",
                _options.SpecFlowPackageVersion);
            InstallNuGetPackage(projectChanger, packagesFolder, "SpecFlow." + _options.UnitTestProvider, sourcePlatform,
                _options.SpecFlowPackageVersion);
            return;
        }
    ...
    {
        var sourcePlatform = GetSpecFlowSourcePlatform();
        InstallNuGetPackage(projectChanger, packagesFolder, "SpecFlow", sourcePlatform,
            _options.SpecFlowPackageVersion);
